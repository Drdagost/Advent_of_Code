rldata = open("2021\Day15_input.txt").read().split()
...
def getPathLength(r):
    op = {(0,0):(0,est_len(0,0,0))} # open dictionary of squares whose neighbors are to be investigated
    cl = dict() # closed dictionary of squares whose neighbors have been investigated
    ...
    while (width*r-1,height*r-1) not in cl:
        ...
        # all neighbors of (x,y) to op{} if they are not already on cl{}
        for (v,w) in getNeighbors(x,y,r):
            if (v,w) not in cl:
                d00 = min(op.get((v,w),(999999999999,0))[0],d+rl(v,w))
                op[v,w] = d00,est_len(v,w,d00,r)

    # return the 'path length from (0,0)' label of the bottom right square
